...
## Implemented
- (2026-08-27) PHP + MySQL package for Hostinger shared hosting: api/ (config.php, db.php with auto table creation, leads.php, admin/login|leads|enrollments.php with HMAC tokens, .htaccess clean-URL rewrites) lives in frontend/public/api so it ships inside the CRA build. Frontend API base falls back to same-origin when REACT_APP_BACKEND_URL is empty. Root .htaccess excludes /api/ from SPA fallback. Deliverables: app2-main.zip (source, for Deployments) and hostinger-upload.zip (prebuilt, for File Manager). DB creds + TOKEN_SECRET filled per user request. Tested locally with php -S + MariaDB: lead insert 200/422, login 200/401, admin lists 200/401.
- (2026-08-21) .htaccess SPA fallback added to frontend/public for Hostinger (fixes 404 on /admin direct links).
- (2026-08-25) Chatbot is now fully client-side: fixed-answer FAQ engine lives in FloatingActions.jsx (14 topics, word-boundary matching with plural tolerance), zero API calls — works on plain static hosting. Off-topic → "contact us" + Talk to the team handoff (lead form still posts to /api/leads). Backend /api/chat endpoint, chat storage, and chat tests removed.
- (2026-08-25) Payment moved off-site to the LMS: all Enroll buttons now redirect to the course portal — Online card → course.onestockacademy.com/new-courses/4-buniyaad-online, Offline card → /new-courses/5-buniyaad-offline, generic CTAs (navbar, hero, banner, final CTA, quiz, footer) → course.onestockacademy.com. Internal /enroll + /payment/result pages and routes deleted (BuyCourse.jsx, PaymentResult.jsx removed); backend order/payment endpoints left dormant for admin enrollment history. Chatbot enroll answer updated to match new flow.
